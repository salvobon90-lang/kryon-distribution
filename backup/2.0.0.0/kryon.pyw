...
VERSION = " 17.0.0 (STRICT PROP DESK)"
...
def log_event(msg):
    ...
    log_box.see(tk.END)
    if int(float(log_box.index('end-1c'))) > 300: log_box.delete("1.0", "2.0")

def log_error(msg):
    timestamp = datetime.now().strftime("%H:%M:%S")
    error_box.insert(tk.END, f"[{timestamp}] ⚠️ {msg}\n")
    error_box.see(tk.END)
    if int(float(error_box.index('end-1c'))) > 100: error_box.delete("1.0", "2.0")

def log_live_analysis(msg):
    live_analysis_box.insert(tk.END, f"{msg}\n")
    live_analysis_box.see(tk.END)
    if int(float(live_analysis_box.index('end-1c'))) > 300:
...
# 🔴 2. CONTROL PANEL (AI) (Row 0, Col 1)
...
# 🔴 4. TELEMETRY DASHBOARD (Row 1, Col 1)
...
# 🔴 5. LOG E SCANNER (Row 2, Col 0) - FIXED: Stacked Vertically via Frame
...
# 🔴 6. ERROR E DEBUG (Row 2, Col 1) - FIXED: Stacked Vertically via Frame
